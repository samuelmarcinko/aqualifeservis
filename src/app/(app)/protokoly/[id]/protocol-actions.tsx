...
import {
  finalizeProtocolAction,
  createProtocolRevisionAction,
  changeProtocolStatusAction,
  sendProtocolAction,
} from "../actions";
...
export function ProtocolActions(props: Props) {
  ...
  return (
    <div className="flex flex-wrap gap-2">
      <a href={`/api/protocols/${props.id}/preview`} target="_blank" rel="noreferrer" className="btn-secondary">
        Náhľad PDF
      </a>

      {!props.locked && (
        <Link href={`/protokoly/${props.id}/upravit`} className="btn-secondary">
          Upraviť
        </Link>
      )}
      ...
      {!props.locked ? (
        ...
        <a href={`/api/protocols/${props.id}/pdf?revision=${props.revision}`} className="btn-primary">
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          const okr = await run(() => finalizeProtocolAction(props.id), "Protokol finalizovaný.");
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          const okr = await run(() => createProtocolRevisionAction(props.id), "Nová revízia vytvorená.");
          if (okr) {
            setConfirmRevision(false);
            router.push(`/protokoly/${props.id}/upravit`);
          }
        }}
      ...
      {sendOpen && (
        <SendModal {...props} onClose={() => setSendOpen(false)} onSent={() => { setSendOpen(false); router.refresh(); }} />
      )}
      {statusOpen && (
        <StatusModal id={props.id} current={props.status} onClose={() => setStatusOpen(false)} onChanged={() => { setStatusOpen(false); router.refresh(); }} />
      )}
    </div>
  );
}
...
function SendModal(props: Props & { onClose: () => void; onSent: () => void }) {
  ...
  async function send() {
    setSending(true);
    const res = await sendProtocolAction(props.id, { recipient, subject, message, revision: props.revision });
    setSending(false);
    if (res.ok) {
...
function StatusModal({ id, current, onClose, onChanged }: { id: string; current: string; onClose: () => void; onChanged: () => void }) {
  const { toast } = useToast();
  ...
  async function save() {
    setSaving(true);
    const res = await changeProtocolStatusAction(id, status);
    setSaving(false);
    if (res.ok) {
