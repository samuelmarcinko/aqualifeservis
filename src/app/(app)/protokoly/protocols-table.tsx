...
import { deleteProtocolAction, deleteProtocolsAction } from "./actions";
...
export function ProtocolsTable({ rows }: { rows: ProtocolRow[] }) {
  const router = useRouter();
  const { toast } = useToast();
  const [selected, setSelected] = useState<Set<string>>(new Set());
  const [deleteOne, setDeleteOne] = useState<ProtocolRow | null>(null);
  ...
  async function doDeleteOne() {
    if (!deleteOne) return;
    const res = await deleteProtocolAction(deleteOne.id);
    if (res.ok) {
      toast("Protokol vymazaný.", "success");
      router.refresh();
    } else toast(res.error, "error");
    setDeleteOne(null);
  }
  async function doDeleteBulk() {
    const res = await deleteProtocolsAction([...selected]);
    if (res.ok) {
    ...
    <div>
      ...
      <div className="table-wrap">
        <table className="tbl">
          ...
          <tbody>
            {rows.map((p) => (
              <tr key={p.id} className={selected.has(p.id) ? "bg-brand/5" : ""}>
                <td>
                  <input type="checkbox" checked={selected.has(p.id)} onChange={() => toggle(p.id)} aria-label={`Vybrať ${p.number}`} />
                </td>
                <td>
                ...
                <td className="text-center">{p.photoCount}</td>
      ...
      <ConfirmDialog
        open={!!deleteOne}
        title="Vymazať protokol?"
        message={`Protokol ${deleteOne?.number} vrátane revízií, fotografií a PDF bude natrvalo vymazaný. Túto akciu nie je možné vrátiť späť.`}
        danger
      ...
      <ConfirmDialog
        open={deleteBulk}
        title="Vymazať vybrané protokoly?"
        message={`Natrvalo bude vymazaných ${selected.size} protokolov vrátane fotografií a PDF. Túto akciu nie je možné vrátiť späť.`}
        danger
        confirmLabel="Vymazať"
