...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const revision = revisionParam
    ? await prisma.repairProtocolRevision.findUnique({
        where: { protocolId_revision: { protocolId: id, revision: Number(revisionParam) } },
        include: { storedDocument: true },
      })
    : await prisma.repairProtocolRevision.findFirst({
        where: { protocolId: id },
